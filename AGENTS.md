...
3. **Executable material (`notebooks/`, `src/`, `tests/`, `sql/`)**
   - Reusable, production-facing logic belongs in `src/datacoding/`; notebooks teach and exercise it.
   - A notebook labeled "from scratch" must also contain the concise implementation it teaches and run without importing this project's package. Keep the hardened `src/` version as the tested reference rather than hiding the learning path behind it.
...
- Put reusable, production-facing implementations in `src/datacoding/`; avoid multiple drifting copies. A from-scratch notebook is the deliberate exception: keep its compact teaching implementation visible, standalone, and behaviorally checked against the same core invariants.
